...
/* Token generation counter for debugging */
static uint64_t luo_token_counter;
static uint64_t luo_token_gen_count;
...
static uint64_t luo_generate_token(const char *path)
{
	...
	luo_token_gen_count++;
	token = (hash ^ (luo_token_counter++ << 48) ^ (luo_token_gen_count << 32));
...
int luo_save_image_metadata(void)
{
	...
	msg->n_fd_mappings = luo_token_gen_count;
